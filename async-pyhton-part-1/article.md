...
The last two paragraphs is essentially the crux of asynchronous programming. You can visualize it as a queue of tasks with one person (the *loop*) assigned to take each one out of the top of it and attend to it. Those tasks that he/she thinks will take a long time to complete, is marked so and put back in the queue to complete (provided the task itself declares that). This process repeats until the person is asked to stop serving the task queue.

## Conclusion
To facilitate the above mode of functioning, what one needs is a function whose state can be suspended and resumed (generator functions, rings a bell?). That means functions whose state (code & data) can be frozen so that it can be resumed later. In programming language theory these are generally referred to as a closure. In the recent years where computing has become network centric & IO intensive (HTTP servers & dynamic webpages driven by backend databases), closures have become a particularly handy tool to scale up servers. That is make the same hardware infrastructure serve more number of clients that what is possible with threads.

This is exactly what the two server examples above demonstrates.

If you have questions or comments, feel free to post them to the FB page where a link to this article will be posted. I'll try to check them daily and answer any questions you may have.
